...
String PIR6_status, DDp1_status, DDp2_status, Wire_cut, Contactron_Status;
String NFC_UID[3] = {"", "", ""};
...
const byte Contactron_A = 2;
const byte Contactron_B = 3;
...
void setup() {
  ...
  pinMode(Contactron_A,INPUT_PULLUP);
  pinMode(Contactron_B,OUTPUT);
...
void loop() {
 ...
 // }
  ...
  //Read Contactron Status
  drum = digitalRead(Contactron_A);
  if(drum) {
    Contactron_Status = "Open";
  } else {
    Contactron_Status = "Close";
  }
    ...
    if (client.available()) {
        switch (client.read()) {
          case 's': //reply with status PIR6, Wire_cut, NFC`s;
            ...
            client.print("Contactron=");
            client.println(Contactron_Status);
            /*
            for (byte i = 0; i < 3; i++) {
